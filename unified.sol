...
contract MultiSigWallet {
    ...
    modifier validRequirement(uint ownerCount, uint _required, PumaPayToken _token) {
        require(
    ...
    function MultiSigWallet(address[] _owners, uint _required, PumaPayToken _token)
        public
    ...
    /// @param executed Include executed transactions.
    /// @return Returns array of transaction IDs.
    function getTransactionIdForAddress(address _destination, uint _value, bool _executed)
        public
        constant
        returns (uint _transactionId)
    {
        Transaction storage tempTransaction = Transaction({
        ...
        for (uint i = 0; i < transactionCount; i++) {            
            if (transactions[i] == tempTransaction) {
